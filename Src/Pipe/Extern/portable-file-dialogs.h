...
namespace pfd
{
	...
	namespace internal
	{
			...
			public:
				...
				public:
					proc(dll const& lib, std::string const& sym)
					    : m_proc(
					        reinterpret_cast<T*>((void*)::GetProcAddress(lib.handle, sym.c_str())))
					{}
		...
		protected:
			...
			file_dialog(type in_type, std::string const& title,
	...
	{
	public:
		message(std::string const& title, std::string const& text,
	...
	namespace internal
	{
		...
		static inline std::wstring str2wstr(std::string const& str)
		{
			...
			MultiByteToWideChar(
			    CP_UTF8, 0, str.c_str(), (int)str.size(), (LPWSTR)ret.data(), (int)ret.size());
			return ret;
		}
		...
		static inline std::string wstr2str(std::wstring const& str)
		{
			int len = WideCharToMultiByte(
			    CP_UTF8, 0, str.c_str(), (int)str.size(), nullptr, 0, nullptr, nullptr);
		...
		static inline bool is_vista()
		{
			...
			return VerifyVersionInfoW(
			           &osvi, VER_MAJORVERSION | VER_MINORVERSION | VER_SERVICEPACKMAJOR, mask)
			    != FALSE;
		}
	#endif
	    ...
	    std::vector<std::string> const& filters /* = {} */, opt options /* = opt::none */)
	{
		...
		m_async->start_func([this, in_type, title, default_path, filter_list, options](
			...
			if (in_type == type::folder)
			{
				...
				if (flags(flag::is_vista))
				{
					...
					IFileDialog* ifd;
					HRESULT hr =
					    dll::proc<HRESULT WINAPI(REFCLSID, LPUNKNOWN, DWORD, REFIID, LPVOID*)>(
					        ole32, "CoCreateInstance")(CLSID_FileOpenDialog, nullptr,
					        CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&ifd));
			...
			if (in_type == type::save)
			{
				...
				dll::proc<BOOL WINAPI(LPOPENFILENAMEW)> get_save_file_name(
			...
			else
			{
				...
				dll::proc<BOOL WINAPI(LPOPENFILENAMEW)> get_open_file_name(
		...
		if (is_osascript())
		{
			...
			switch (in_type)
			{
				case type::save: script += " file name"; break;
				case type::open:
				default:
					script += " file";
					if (options & opt::multiselect)
	...
	inline std::string internal::file_dialog::select_folder_vista(IFileDialog* ifd, bool force_path)
	{
		...
		// Load library at runtime so app doesn't link it at load time (which will fail on windows
		// XP)
	    ...
	    std::string const& title, std::string const& message, icon _icon /* = icon::info */)
	{
		...
		StringCchCopyW(
		    nid->szInfoTitle, ARRAYSIZE(nid->szInfoTitle), internal::str2wstr(title).c_str());
	    ...
	    choice _choice /* = choice::ok_cancel */, icon _icon /* = icon::info */)
	{
		...
		#define PFD_OSX_ICON(n)                                           \
			...
			"& \"CoreServices:CoreTypes.bundle:Contents:Resources:" n ".icns\")"
				case icon::info:
				default: script += PFD_OSX_ICON("ToolBarInfo"); break;
		...
		else if (is_zenity())
		{
			switch (_choice)
			{
				case choice::ok_cancel:
					command.insert(
					    command.end(), {"--question", "--cancel-label=Cancel", "--ok-label=OK"});
					break;
				case choice::yes_no:
				...
				case choice::yes_no_cancel:
					command.insert(
					    command.end(), {"--question", "--switch", "--extra-button=Cancel",
				...
				case choice::abort_retry_ignore:
					command.insert(
					    command.end(), {"--question", "--switch", "--extra-button=Ignore",
	...
	inline open_file::open_file(std::string const& title, std::string const& default_path,
	    std::vector<std::string> const& filters, bool allow_multiselect)
	    : open_file(
	        title, default_path, filters, (allow_multiselect ? opt::multiselect : opt::none))
	{}
	...
	inline save_file::save_file(std::string const& title, std::string const& default_path,
	    std::vector<std::string> const& filters, bool confirm_overwrite)
	    : save_file(
	        title, default_path, filters, (confirm_overwrite ? opt::none : opt::force_overwrite))
	{}
	...
	inline select_folder::select_folder(std::string const& title,
	    std::string const& default_path /* = "" */, opt options /* = opt::none */)
