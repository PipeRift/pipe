...
namespace Rift
{
	...
	namespace Strings
	{
		...
		{
			String str;
			fmt::vformat_to(std::back_inserter(str), format, fmt::make_format_args(std::forward<Args>(args)...));
			return Move(str);
		}
		...
		{
			fmt::vformat_to(std::back_inserter(buffer), format, fmt::make_format_args(std::forward<Args>(args)...));
		}
