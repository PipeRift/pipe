...
namespace p
{
	...
	struct P_API SortLessStatics{
	    bool operator()(const OwnPtr& a, const OwnPtr& b) const {return a.GetId() < b.GetId();
}    // namespace p
...
public:
	...
	template<typename... C>
	void AddN(TView<const Id> ids) const requires((IsSame<C, Mut<C>> && ...) && sizeof...(C) > 1)
	{
		(AddN<C>(ids), ...);
	}
...
P_API void ExcludeIdsWith(const BasePool* pool, TArray<Id>& ids, const bool shouldShrink = true);
...
P_API void ExcludeIdsWithout(const BasePool* pool, TArray<Id>& ids, const bool shouldShrink = true);
...
P_API void ExtractIdsWith(
    const BasePool* pool, TArray<Id>& source, TArray<Id>& results, const bool shouldShrink = true);
 ...
 */
template<typename C, typename AccessType>
void ExcludeIdsWithStable(const AccessType& access, TArray<Id>& ids, const bool shouldShrink = true)
{
 ...
 */
template<typename C, typename AccessType>
void ExcludeIdsWithout(const AccessType& access, TArray<Id>& ids, const bool shouldShrink = true)
{
 ...
 */
template<typename... C, typename AccessType>
void FindAllIdsWithAnyUnique(const AccessType& access, TArray<Id>& ids) requires(sizeof...(C) >= 1)
{
